...
def filter_step(old_pose, motor_ticks, ticks_to_mm, robot_width,
    ...
    if motor_ticks[0] == motor_ticks[1]:
        ...
        x = old_pose[0] + ( motor_ticks_right )* cos(old_pose[2])
        y = old_pose[1] + ( motor_ticks_right )* sin(old_pose[2])
        theta = old_pose[2] % (2*pi)
        ...
        R = motor_ticks_left / alpha
        cx = old_pose[0] + scanner_displacement * cos (old_pose[2]) - (R + robot_width/2) * sin(old_pose[2])
        cy = old_pose[1] + scanner_displacement * sin (old_pose[2]) - (R + robot_width/2) * (-cos(old_pose[2]))
        theta = (old_pose[2] + alpha)%(2*pi)
        x = cx - scanner_displacement * cos (old_pose[2]) + (R + robot_width/2) * sin (theta)
        y = cy - scanner_displacement * sin (old_pose[2]) + (R + robot_width/2) * (-cos ( theta))
