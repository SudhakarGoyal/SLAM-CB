...
def kalman_plot(prediction, measurement, correction):
    """Helper to draw all curves in each filter step."""
    plot([scs.norm.pdf(x, prediction.mu, sqrt(prediction.sigma2))
          for x in range(*arena)], color = 'b', linewidth=2)
    plot([scs.norm.pdf(x, measurement.mu, sqrt(measurement.sigma2))
          for x in range(*arena)], color = 'g', linewidth=2)
    plot([scs.norm.pdf(x, correction.mu, sqrt(correction.sigma2))
...
def kalman_filter_step(belief, control, measurement):
    ...
    correction =  Density( prediction.mu + K*(measurement.mu - prediction.mu), (1-K)*prediction.sigma2 ) # Replace
