...
def get_corresponding_points_on_wall(points,
    ...
    for i in range(len(points)):
        if ((points[i][0] > arena_left - eps and points[i][0] < arena_left + eps)):
            x_ = arena_left#points[i][0] - (points[i][0] - arena_left)
            y_ = points[i][1] 
            left_list.append(points[i])
            right_list.append((x_, y_)) 
        if ((points[i][0] > arena_right - eps and points[i][0] < arena_right + eps)):
            x_ = arena_right#points[i][0] + (arena_right - points[i][0])
            y_ = points[i][1]    
            left_list.append(points[i])
            right_list.append((x_, y_)) 
        if ((points[i][1] > arena_top - eps and points[i][1] < arena_top + eps)):
            y_ =  arena_top#points[i][1] + (-points[i][1] + arena_top)
            x_ = points[i][0]
            left_list.append(points[i])
            right_list.append((x_, y_)) 
        if ((points[i][1] > arena_bottom - eps and points[i][1] < arena_bottom + eps)):
            y_  =  arena_bottom#points[i][1] - (points[i][1] - arena_bottom) 
            x_ = points[i][0]
            left_list.append(points[i])
            right_list.append((x_, y_))     
                   
    # ---> Implement your code here.        break
